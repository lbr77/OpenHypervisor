 ...
 *
 * These used to be read where they were asked, behind a thread-local cache,
 ...
 * touch is a call into libdyld -- and the rest still reaching getenv, whose
 * `__findenv_locked` is a lock six vcpu threads then queue on.  This file asks
 * 58 questions of the environment and several of them are on the path of every
 * trapped system register.
 *
 * They are configuration.  They cannot change while a VM is running, so they
 * are read at load, before any vcpu exists, and a read afterwards is a load
 * from a global.  The `cfg_` prefix keeps the field names clear of the
 * same-named macros elsewhere in the tree.
 */
struct OhvCfg {
